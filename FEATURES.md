...
- **Drawer tab swiping** — swipe left/right inside the drawer to switch between Sessions and Agents tabs with a sliding animation (30px threshold, velocity-aware)
...
- **Start anywhere** — select text in a message or document and a **▶ Play from here** pill appears; tap it to start at that block. (Selecting first and then pressing the message's play button, or the files sheet's ▶ Listen, does the same thing.) If something is already playing and the selected block is part of it, the pill seeks instead of restarting — so it doubles as a precise scrub
