...
- **Compose panel** — tap the pencil icon to open a full-width auto-growing textarea above the keyboard. The `+` button (inset bottom-left) opens an action sheet to **Take Photo**, **Photo Library**, or **Attach File** — multiple images attach as thumbnails with tap-to-preview, long-press-to-replace, and `×` to remove. The send button (↑ circle, bottom-right) dispatches all queued images and text as a **single combined prompt** (joined with line breaks). Multi-line text is supported — use Ctrl+J to insert a line break in the textarea. Type `/` to get an inline slash command popup showing built-in Claude Code commands (`/compact`, `/clear`, `/help`, `/bug`, `/cost`, `/doctor`, `/init`, `/login`, `/logout`, `/memory`, `/model`, `/pr_comments`, `/release-notes`, `/review`, `/status`, `/terminal-setup`, `/vim`) merged with any session-specific plugin commands (e.g. GSD skills). Supports substring and subsequence matching — typing `/cmpct` finds `/compact`
