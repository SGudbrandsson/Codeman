  ...
  await page.route(`**/api/sessions/${sessionId}/transcript**`, (route) => {
    ...
    if (tail > 0 && arr.length > tail) {
      route.fulfill({
        ...
        body: JSON.stringify(arr.slice(-tail)),
      });
    } else {
...
describe('_renderTailBatch: large transcript (lazy loading active)', () => {
  ...
  it('_renderedStartIdx equals fetchedCount - BATCH_SIZE (tail pagination)', async () => {
    const FETCHED = Math.min(TOTAL_BLOCKS, BATCH_SIZE * 2);
    const idx = await getRenderedStartIdx(page);
    expect(idx).toBe(FETCHED - BATCH_SIZE);
  });

  it('first rendered block is from the tail of the fetched window', async () => {
    const text = await getFirstBlockText(page);
    const FETCHED = Math.min(TOTAL_BLOCKS, BATCH_SIZE * 2);
    const fetchStart = TOTAL_BLOCKS - FETCHED;
    const expectedIdx = fetchStart + (FETCHED - BATCH_SIZE);
...
describe('_prependBatch: loading older messages on scroll-up', () => {
  ...
  it('prepend batch adds older blocks and updates _renderedStartIdx', async () => {
    ...
    expect(idxBefore).toBe(FETCHED - BATCH_SIZE);
    ...
    const count = await getRenderedBlockCount(page);
    expect(count).toBe(FETCHED);
  });

  it('sentinel remains because server has more blocks than local cache', async () => {
    const sentinel = await hasSentinel(page);
    expect(sentinel).toBe(true);
  });

  it('oldest rendered block is the first block of the fetched tail', async () => {
    const text = await getFirstBlockText(page);
    const expectedIdx = TOTAL_BLOCKS - Math.min(TOTAL_BLOCKS, BATCH_SIZE * 2);
    expect(text).toContain(`Message ${expectedIdx}`);
  });

  it('server-side prepend fetches more blocks and renders them', async () => {
    await triggerPrependBatch(page);
    await page.waitForTimeout(300);

    const stateBlockCount = await page.evaluate((sid: string) => {
      const tv = window as unknown as { TranscriptView: { _getState: (id: string) => { blocks: unknown[] } } };
      return tv.TranscriptView._getState(sid).blocks.length;
    }, sessionId);
    expect(stateBlockCount).toBeGreaterThan(BATCH_SIZE * 2);
  });
});
