...
describe('RespawnController onClear callback', () => {
  let session: MockSession;
  ...
  it('calls onClear callback instead of writing /clear to PTY', async () => {
    const onClear = vi.fn().mockResolvedValue(undefined);
    const writeViaMuxSpy = vi.spyOn(session, 'writeViaMux').mockResolvedValue(true);

    const ctrl = new RespawnController(session as unknown as Session, {
      idleTimeoutMs: 50,
      interStepDelayMs: 10,
      sendClear: true,
      sendInit: false,
    ...
    ctrl.start();
    ctrl.emit('stepCompleted', 'update');
    await new Promise((resolve) => setTimeout(resolve, 150));
    ...
    if (onClear.mock.calls.length > 0) {
      ...
      expect(writeViaMuxSpy).not.toHaveBeenCalledWith('/clear\r');
    }
    writeViaMuxSpy.mockRestore();
  });

  it('RespawnConfig accepts onClear field', () => {
    const onClear = async () => {};
    const config: RespawnConfig = {
      enabled: true,
      idleTimeoutMs: 100,
      updatePrompt: 'test',
      interStepDelayMs: 50,
