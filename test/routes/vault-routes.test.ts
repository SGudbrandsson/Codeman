...
describe('vault-routes', () => {
  ...
  describe('GET /api/agents', () => {
    it('returns empty list when no agents exist', async () => {
      const res = await harness.app.inject({ method: 'GET', url: '/api/agents' });
    ...
    it('returns list of existing agents', async () => {
      harness.ctx.store.setAgent(makeProfile('agent-list-1', vaultPath));
      harness.ctx.store.setAgent(makeProfile('agent-list-2', vaultPath));
  ...
  describe('GET /api/agents/:agentId', () => {
    it('returns agent profile for existing agent', async () => {
      harness.ctx.store.setAgent(makeProfile('agent-get-1', vaultPath));
      const res = await harness.app.inject({ method: 'GET', url: '/api/agents/agent-get-1' });
      expect(res.statusCode).toBe(200);
      const body = JSON.parse(res.body);
      expect(body.success).toBe(true);
      expect(body.agent.agentId).toBe('agent-get-1');
    });

    it('returns error for unknown agent', async () => {
      const res = await harness.app.inject({ method: 'GET', url: '/api/agents/no-such-agent' });
      const body = JSON.parse(res.body);
  ...
  describe('POST /api/agents', () => {
    it('creates a new agent with required fields', async () => {
      const postVault = makeTmpVault('post');
      try {
        ...
        const body = JSON.parse(res.body);
        expect(body.success).toBe(true);
        expect(body.agent.agentId).toBe('new-agent-1');
        expect(body.agent.displayName).toBe('New Agent');
        expect(body.agent.notesSinceConsolidation).toBe(0);
      } finally {
        rmSync(postVault, { recursive: true, force: true });
      }
    });
    ...
    it('returns error when agentId is missing', async () => {
      const res = await harness.app.inject({
        method: 'POST',
        url: '/api/agents',
        payload: { role: 'implementer', displayName: 'No ID Agent' },
      });
      const body = JSON.parse(res.body);
      expect(body.success).toBe(false);
    });

    it('returns error when agent already exists', async () => {
      harness.ctx.store.setAgent(makeProfile('duplicate-agent', vaultPath));
      const res = await harness.app.inject({
        method: 'POST',
        url: '/api/agents',
        payload: { agentId: 'duplicate-agent', role: 'implementer', displayName: 'Dup' },
      });
      const body = JSON.parse(res.body);
      expect(body.success).toBe(false);
    });
  ...
  describe('PATCH /api/agents/:agentId', () => {
    it('updates displayName', async () => {
      harness.ctx.store.setAgent(makeProfile('patch-agent', vaultPath));
      const res = await harness.app.inject({
        method: 'PATCH',
        url: '/api/agents/patch-agent',
        payload: { displayName: 'Updated Name' },
      });
      const body = JSON.parse(res.body);
      expect(body.success).toBe(true);
      expect(body.agent.displayName).toBe('Updated Name');
    });

    it('prevents changing agentId via PATCH', async () => {
      harness.ctx.store.setAgent(makeProfile('immutable-agent', vaultPath));
      const res = await harness.app.inject({
        method: 'PATCH',
        url: '/api/agents/immutable-agent',
        payload: { agentId: 'hijacked-id' },
      });
      const body = JSON.parse(res.body);
      expect(body.success).toBe(true);
      expect(body.agent.agentId).toBe('immutable-agent');
    });
  ...
  describe('DELETE /api/agents/:agentId', () => {
    it('deletes an existing agent and returns 204', async () => {
      harness.ctx.store.setAgent(makeProfile('del-agent', vaultPath));
      const res = await harness.app.inject({ method: 'DELETE', url: '/api/agents/del-agent' });
      expect(res.statusCode).toBe(204);
    });

    it('returns error for unknown agent', async () => {
      const res = await harness.app.inject({ method: 'DELETE', url: '/api/agents/no-such' });
      const body = JSON.parse(res.body);
      expect(body.success).toBe(false);
    });
  });
