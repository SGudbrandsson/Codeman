 ...
 * execFile is used (no shell) so the validated name is passed as a literal argument.
 */
 ...
 */
export async function captureScreen(muxName: string): Promise<string | null> {
  if (!/^codeman-[a-f0-9-]+$/.test(muxName)) return null;
  try {
    const { stdout } = await execFileAsync('tmux', ['capture-pane', '-p', '-e', '-t', muxName], {
      encoding: 'utf-8',
      timeout: 5000,
    });
    return stdout;
