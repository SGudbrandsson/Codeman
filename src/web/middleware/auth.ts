...
export function registerSecurityHeaders(app: FastifyInstance, https: boolean): void {
  app.addHook('onRequest', (req, reply, done) => {
    ...
    reply.header(
      ...
      "default-src 'self'; script-src 'self' 'unsafe-inline' 'wasm-unsafe-eval' https://cdn.jsdelivr.net; style-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net; img-src 'self' data: blob:; connect-src 'self' wss://api.deepgram.com https://api.deepgram.com; media-src 'self' blob: data:; font-src 'self' https://cdn.jsdelivr.net; frame-ancestors 'self'"
    );
    if (https) {
